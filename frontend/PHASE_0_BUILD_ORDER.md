...
>
> **Visual direction:** minimalist, editorial, "tech-first hospital-second" (see _Visual Design Language_ below).
...
- [x] Astro app builds and runs (`npm run dev` / `npm run build`). _(output: static, no adapter needed)_
...
- [x] `api/lead.ts` validates and forwards to CRM; WhatsApp deep link works. _(static: direct external API via `PUBLIC_CRM_SUBMIT_URL`)_
