 ...
 *
 * On success: the form modal closes immediately and a centered success
 * overlay appears (LeadSuccessOverlay) so the visitor gets clean feedback
 * with unlimited time to read it and click WhatsApp/Email.
 */
function LeadFormModal({
  ...
  // Reason: track success state so we can show the overlay after the modal closes.
  ...
  // Reason: reset success state when the overlay is dismissed.
  ...
  return (
    <>
      {/* Form modal — hidden when success overlay is showing */}
      {open &&
        !showSuccess &&
        createPortal(
          <div
            ...
            >
              ...
              <div className="overflow-y-auto px-5 py-5 md:px-6 md:py-6">
                <LeadForm
                  ...
                  onSuccess={(data) => {
                    // Reason: close the form modal immediately and show
                    // the centered success overlay with the visitor's data
                    // so they have unlimited time to read it and click
                    // WhatsApp/Email.
                    setSuccessData(data);
                    setShowSuccess(true);
                    onClose();
      ...
      {/* Success overlay — centered, dismissed explicitly by user */}
      {showSuccess && (
        <LeadSuccessOverlay
          open={showSuccess}
          onClose={handleSuccessClose}
          whatsappHref={successData?.whatsappHref ?? ''}
          contactEmail={
            typeof import.meta !== 'undefined' && import.meta.env
              ? import.meta.env.PUBLIC_CONTACT_EMAIL || 'contact@khanmeditour.com'
              : 'contact@khanmeditour.com'
          }
          submittedName={successData?.submittedName ?? ''}
        />
      )}
    </>
  );
